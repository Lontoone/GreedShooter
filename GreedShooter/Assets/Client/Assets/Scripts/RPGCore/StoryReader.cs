...
public class StoryReader : MonoBehaviour
{
    ...
    /*
    public IEnumerator LoadSceneWithObject(string sceneName, GameObject playerToMove, string spawnPointName)
    {
        ...
        {
            Vector3 _targetPos = GameObject.Find(MapChanger.teleport_obj_name).transform.position;
            playerToMove.transform.position = _targetPos;
        }

    }

    public IEnumerator UnloadScene(string unloadName, string exceptSceneName)
    {
        if (unloadName == "all")
        {
            //紀錄要刪除的scene
            ...
            for (int i = 0; i < scenesToUnload.Count; i++)
            {
                ...
                AsyncOperation async = SceneManager.UnloadSceneAsync(unloadName);
                while (!async.isDone)
                    yield return null;
            }
        }
